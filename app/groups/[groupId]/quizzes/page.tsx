	...
	return (
		<RequireAuth>
			<div className="flex flex-col h-full w-full bg-white relative overflow-hidden p-12">
				{/* Maximalist Backdrop */}
				<div className="absolute top-0 right-0 text-[35vw] font-black uppercase text-black/[0.01] select-none pointer-events-none -translate-y-1/4 translate-x-1/4 leading-none">
					TEST
				</div>
					...
					<div className="grid md:grid-cols-12 gap-12 overflow-y-auto pb-32 pr-4 custom-scrollbar">
						...
						<div className="md:col-span-5 flex flex-col gap-8 bg-black/5 p-12 rounded-[3.5rem] h-fit">
							<div className="flex flex-col gap-2">
								<h3 className="text-xs font-black uppercase tracking-[0.3em] text-deepInk/40">01. Studio Creator</h3>
								<h4 className="text-3xl font-black uppercase tracking-tighter text-deepInk">Create Quiz</h4>
							</div>
							...
							<div className="flex flex-col gap-6">
								<input 
									...
									className="w-full bg-white rounded-full px-8 py-5 text-[10px] font-black uppercase tracking-widest border-0 focus-ring shadow-soft" 
								/>
										...
										{new Array(4).fill(0).map((_, idx) => (
											<div key={idx} className="flex items-center group">
												<input
													...
													className="flex-1 bg-white rounded-full px-8 py-4 text-[10px] font-black uppercase tracking-widest border-0 focus-ring shadow-soft"
												/>
												<button
								...
								{builderQs.length > 0 && (
									<div className="flex items-center justify-between px-2">
										<span className="text-[8px] font-black uppercase tracking-[0.3em] opacity-40">Questions Cached: {builderQs.length}</span>
						...
						<div className="md:col-span-7 flex flex-col gap-10">
							<div className="flex flex-col gap-2">
								<h3 className="text-xs font-black uppercase tracking-[0.3em] text-deepInk/40 px-4">02. Live Library</h3>
								<h4 className="text-3xl font-black uppercase tracking-tighter text-deepInk px-4">Deployments</h4>
							</div>
							...
							<div className="flex flex-col gap-8">
								{quizzes.map((q) => (
									<div key={q.id} className="p-10 bg-white border border-black/5 rounded-[3rem] shadow-soft hover:shadow-max group transition-all duration-700 flex items-center justify-between">
										<div className="flex flex-col gap-2">
											<div className="text-2xl font-black uppercase tracking-tighter text-deepInk group-hover:text-neonLime transition-colors">{q.title}</div>
											<div className="text-[10px] font-bold text-deepInk/30 uppercase tracking-widest">{q.questions.length} Items Pool</div>
										</div>
								...
								{activeQuiz && (
									<div className="p-16 bg-deepInk rounded-[4rem] text-white shadow-max mt-12 relative overflow-hidden transition-all duration-700">
										<div className="absolute top-0 right-0 text-[10vw] font-black text-white/[0.03] select-none pointer-events-none uppercase leading-none">QUIZ</div>
										<h3 className="text-4xl font-black uppercase tracking-tighter mb-16 relative z-10">{activeQuiz.title}</h3>
										
										<div className="space-y-16 relative z-10">
											{activeQuiz.questions.map((q, i) => (
												<div key={i} className="space-y-8 border-b border-white/5 pb-12 last:border-0">
													<div className="text-xs font-black uppercase tracking-[0.5em] text-white/40">Question 0{i + 1}</div>
													<div className="text-2xl font-black uppercase tracking-tighter leading-tight italic">{q.text}</div>
											...
											<div className="flex items-center justify-between pt-8">
												...
												{result !== null && (
													<div className="text-right">
														<div className="text-xs font-black uppercase tracking-[0.3em] text-white/40 mb-1">Final Score</div>
